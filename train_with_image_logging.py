...
def get_img_from_example(parsed_example, intensify=True):
  ...
  for i, band in enumerate(['B4', 'B3', 'B2']):
    ...
    #rgbArray[..., i] = band_data.numpy()
    bandlist.append(band_data)
  rgbArray = tf.concat(bandlist, -1)#) #, -1)
...
def train_cnn(args):
  ...
  train_images, train_labels = parse_tfrecords(train_tfrecords, args.batch_size, LIMIT_TR)
  val_images, val_labels = parse_tfrecords(val_tfrecords, args.batch_size, LIMIT_VL)
  val_viz, val_viz_labels = parse_tfrecords(val_tfrecords, args.batch_size, 10, include_viz=True)
  print(val_viz["image"].shape)
 
  wandb.log({"examples" : [wandb.Image(v.numpy()) for v in val_viz["image"]]})
#[wandb.Image(v) for v in val_viz]}) #[0], caption=v[1]) for v in viz_ex]})
  
  # try training faster
  #train_images = train_images[:LIMIT]
  #train_labels = train_lables[:LIMIT]
  ...
  train_steps_per_epoch = int(math.floor(float(LIMIT_TR) /float(args.batch_size)))
  #val_steps_per_epoch = int(math.floor(float(NUM_VAL)/float(args.batch_size)))
  val_steps_per_epoch = int(math.floor(float(LIMIT_VL)/float(args.batch_size)))
